...
export const stripeWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    const stripe = new Stripe(stripeSecret.value(), { apiVersion: '2024-04-10' as any });
...
async function handleShippingPayment(
  db: DB,
  session: Stripe.Checkout.Session
): Promise<void> {
  const orderId = session.metadata?.orderId;
  if (!orderId) { console.error('Shipping payment: no orderId in metadata'); return; }
  const orderRef = db.collection('orders').doc(orderId);
  const quoteRef = db.collection('shipping_quotes').doc(orderId);
  await Promise.all([
    orderRef.update({
      shippingStatus: 'ready_to_ship',
      timeline: FieldValue.arrayUnion({ event: 'shipping_paid', at: Timestamp.now() }),
      updatedAt: FieldValue.serverTimestamp(),
    }),
    quoteRef.update({ status: 'paid', paidAt: FieldValue.serverTimestamp() }),
  ]);
...
async function handleSessionCompleted(
  ...
  const orderId = session.metadata?.orderId;
  if (!orderId) { console.error('No orderId in session metadata'); return; }

  const orderRef = db.collection('orders').doc(orderId);
  const orderSnap = await orderRef.get();
  if (!orderSnap.exists) { console.error('Order not found:', orderId); return; }
  ...
  console.log(`Order ${orderId} (${order.orderNumber as string}) confirmed via Stripe`);

  // Confirmation email via Resend
  const toEmail =
    (order.shippingAddress as any)?.email || (order.guestEmail as string | null);
  if (toEmail && resendApiKey) {
    try {
      const resend = new Resend(resendApiKey);
      const firstName = (order.shippingAddress as any)?.firstName ?? 'Cliente';
      await resend.emails.send({
        from: 'The Blondes Concept <ordini@theblondes.it>',
        to: toEmail as string,
        subject: `Ordine ${order.orderNumber as string} confermato – The Blondes Concept`,
        html: buildOrderConfirmationEmail(firstName as string, order),
      });
    } catch (err: unknown) {
...
function buildOrderConfirmationEmail(firstName: string, order: any): string {
    ...
    <div style="font-family:-apple-system,sans-serif;max-width:560px;margin:0 auto;color:#1a1a1a;padding:32px 24px">
      ...
      <div style="background:#fafafa;border:1px solid #e8e8e8;padding:24px;margin-bottom:24px">
        <p style="font-size:11px;text-transform:uppercase;letter-spacing:0.12em;color:#888;margin:0 0 20px">
          Ordine <strong style="color:#1a1a1a">${order.orderNumber as string}</strong>
        </p>
        <table style="width:100%;border-collapse:collapse;border-top:1px solid #e8e8e8">
          ...
          <tr style="border-top:1px solid #e8e8e8">
            ...
            <td style="padding:12px 0;text-align:right;font-size:14px;font-weight:600">
              €${((order.totals.total as number) / 100).toFixed(2)}
            </td>
          </tr>
        </table>
      </div>
